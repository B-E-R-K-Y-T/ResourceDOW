...
local	RTT_SceneData_AppData_Index = 0x41dd73d5 -- used to store data on the scene root node. Using 4 indices starting at this value. 
...
--						  	((not hasModifier_SubObj) and mapChannel == bakeChannel_SubObj) )then
						(
							...
							local paramsMatch =	( unwrapMod.getPreventFlattening() == false ) and 
												( unwrapMod.getFlattenAngle() == flattenAngle ) and
...
(
	...
	if _debug do format "ReadSceneData: % : % : % : % - time:%\n" RTT_data.FileOutput_FilePath RTT_data.Renderer_DisplayFB RTT_data.Renderer_NetworkRender RTT_data.Renderer_SkipExistingFiles (timestamp())
